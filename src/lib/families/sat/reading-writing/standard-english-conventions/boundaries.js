(function (root, factory) {
  ...
  // (`verbs`), which "and" joins with no comma.
  const JOIN_CLAUSE_TOPICS = [
    ...
    },
    {
      ...
      w1: "factor", w2: "fewer", c1: ["Most renters", "ranked"], c2: ["one in ten", "listed"],
    },
    {
    ...
    },
    {
      ...
      w1: "heat", w2: "the", c1: ["Ash", "settles"], c2: ["glaze", "is"],
    },
    {
  ...
  const joinIndependentClauses = {
    ...
    difficulty: "Easy",
    title: "Comma before “and”: two clauses or two verbs",
    recognize:
      "Check what follows the blank: a new subject and verb (a second independent clause, joined by a comma and “and”) or just a second verb for the same subject (joined by “and” alone).",
    ...
    build(t) {
      ...
      const { w1, w2 } = topic;
      const clauses = Boolean(topic.c2);
      const render = (comma, conj) => `${w1}${comma === "yes" ? "," : ""}${conj === "yes" ? " and" : ""} ${w2}`;
      const second = clauses ? `a second independent clause, with its own subject and verb (${topic.c2[0]} ${topic.c2[1]})` : `a second verb, ${q(topic.verbs[1])}, for the same subject, ${q(topic.subject)}`;
      const choices = squareOf(["comma", "conjunction"], [["yes", "no"], ["yes", "no"]], render, (comma, conj) => {
        const text = render(comma, conj);
        if (clauses) {
          if (comma === "yes" && conj === "yes") return null;
        ...
        if (comma === "no" && conj === "yes") return null;
      ...
      const instance = multipleChoice(topic, choices, {
        explanation: clauses
          ? `The words after the blank form ${second}, so the blank joins two independent clauses. A comma followed by "and" joins them: "${choices.correct}".`
          : `The words after the blank begin ${second}; there is no new subject. "And" joins two verbs of one subject without a comma: "${choices.correct}".`,
        steps: [
          "Look at the words right after the blank.",
          "Ask whether they bring a new subject and verb, or only a second verb for the subject already named.",
          "Two clauses: comma plus “and.” Two verbs of one subject: “and” with no comma.",
        ],
        principles: [
        ...
        trap: clauses
          ? "Leaving out the comma before “and,” or keeping the comma but dropping “and,” which creates a comma splice."
          : "Adding a comma before “and” out of habit, though what follows is only a second verb, not a new clause.",
        hint: "Does a new subject follow the blank, or only a new verb?",
        estimatedSeconds: 50,
      });
      instance.verify = () => {
        ...
        const structure = clauses
          ? lead.includes(topic.c1[0]) && lead.includes(topic.c1[1]) && tail.includes(topic.c2[0]) && tail.includes(topic.c2[1]) &&
            !SUBORDINATORS.includes(firstWord(lead))
          : lead.includes(topic.subject) && lead.includes(` ${topic.verbs[0]} `) && w2 === topic.verbs[1];
        return structure && instance.correct === render(clauses ? "yes" : "no", "yes") && isSquare(instance.features);
      };
      return instance;
    },
  };
  ...
  const COLON_LIST_TOPICS = [
    ...
    },
    {
      ...
      text: "Agronomist Lucía Beltrán asked coffee farmers in Colombia's Huila region how they were adapting to rising temperatures. Most farmers described the same three ______ to hardier varieties, and moving their plots to higher ground.",
    ...
    },
    {
      ...
      text: "Whistled languages let speakers communicate across valleys where shouting would not carry. Linguist Pablo Serrano has recorded whistled speech in three ______ Turkish village of Kuşköy, and the mountains of Oaxaca in Mexico.",
      w1: "places", first: "the island of La Gomera", next: "the", count: 3, clause: ["Pablo Serrano", "has recorded"],
    },
    {
  ...
  const INTRO_CLAUSE_TOPICS = [
    ...
    },
    {
      ...
      text: "Violinist Mei Tanaka learns every new concerto slowly. She practices it at half speed for several weeks until she can play each passage ______ she raises the tempo a little each day until she reaches full speed.",
      kind: "joined", conj: "and", w1: "cleanly", w2: "then", first: ["She", "practices"], main: ["she", "raises"],
    },
    {
      scene: "sec-bridge-expansion",
      text: "Engineers attached sensors to a steel bridge in Minneapolis to record how it responds to changes in temperature. The bridge's deck expands by several centimeters as the air warms each ______ it shrinks back again after sunset.",
    ...
    },
    {
      ...
      text: "The city of Linwood pays homeowners to replace part of their lawns with rain gardens planted with native grasses. A garden qualifies for payment once it covers ten square ______ the city inspects each one before sending a check.",
      kind: "joined", conj: "but", w1: "meters", w2: "the", first: ["garden", "qualifies"], main: ["city", "inspects"],
    },
    {
  ...
  // Each topic: two independent clauses, the first of them the passage's
  // first sentence; the second opens with the transition `adv`. The choices
  // cross the mark before `adv` with the mark after it. ", adv;" would make
  // the transition end the first clause and link it to something before it,
  // which does not exist, so only "; adv," is right.
  const ADVERB_JOIN_TOPICS = [
    {
      scene: "sec-lowland-pikas",
      text: "Ecologist Dana Whitcomb expected American pikas, small relatives of rabbits that overheat easily, to vanish from low-elevation slopes in Nevada as summers grew ______ she found thriving colonies living among cool, ice-filled piles of rock well below the predicted limit.",
      w1: "hotter", adv: "however", c1: ["Dana Whitcomb", "expected"], c2: ["she", "found"],
      link: "contrasts what Whitcomb expected with what she found",
    },
    {
    ...
    },
    {
      ...
      text: "Several earlier studies of children who speak two languages had reported that they switch between tasks much faster than children who speak ______ psychologist Marco Ricci's far larger study found only a small difference between the two groups.",
      w1: "one", adv: "in contrast", c1: ["studies", "had reported"], c2: ["study", "found"],
      link: "contrasts Ricci's result with the earlier studies",
    },
    {
      scene: "sec-norwood-street-trees",
      text: "The ten thousand trees that the city of Norwood planted along its streets between 2010 and 2020 shade sidewalks and cool the air around ______ they absorb rainwater that would otherwise overwhelm the city's storm drains.",
      w1: "them", adv: "moreover", c1: ["trees", "shade"], c2: ["they", "absorb"],
      link: "adds a second benefit of the trees to the first",
    },
    {
    ...
    },
    {
      ...
      link: "presents the long gaps between nests as a result of the parrot's dependence on rimu fruit",
    },
    {
    ...
    },
    {
      ...
      text: "Octopuses kept in aquariums have learned to unscrew the lids of glass jars to reach the food ______ a few have escaped their tanks at night to raid neighboring ones for food.",
      w1: "inside", adv: "in fact", c1: ["Octopuses", "have learned"], c2: ["a few", "have escaped"],
      link: "introduces an even more striking case of the octopuses' skill",
    },
    {
    ...
    },
    {
      ...
      text: "The protected bike lanes that twelve North American cities built between 2010 and 2020 cut injuries among cyclists by nearly ______ the number of people cycling to work rose in every one of those cities.",
      w1: "half", adv: "furthermore", c1: ["lanes", "cut"], c2: ["the number", "rose"],
      link: "adds a second result of the lanes to the first",
    },
  ];
  ...
  const conjunctiveAdverbJoin = {
    ...
    recognize:
      "A transition such as however cannot join two independent clauses on its own; the clauses need a semicolon, and the transition belongs to the clause whose relation it names.",
    ...
    build(t) {
      ...
      const render = (before, after) => `${w1}${before === "semicolon" ? ";" : ","} ${adv}${after === "comma" ? "," : ";"}`;
      const choices = squareOf(["before", "after"], [["semicolon", "comma"], ["comma", "semicolon"]], render, (before, after) => {
        if (before === "semicolon" && after === "comma") return null;
        if (before === "semicolon") return `With a semicolon on each side, "${adv}" is stranded between the two clauses and belongs to neither.`;
        if (after === "comma") return `A comma before "${adv}" joins two independent clauses with only a comma, a comma splice; "${adv}" is a transition, not a conjunction like "and" or "but."`;
      ...
      const instance = multipleChoice(topic, choices, {
        explanation: `"${first}" and the words after the blank are both independent clauses, and "${adv}" ${topic.link}, so it opens the second clause. A transition cannot join clauses by itself: a semicolon separates the clauses, and a comma follows the transition: "${choices.correct}".`,
        steps: [
          "Check that the words on each side of the blank are independent clauses.",
          `Decide which clause "${adv}" belongs to: it ${topic.link}.`,
          `Put a semicolon between the clauses and a comma after "${adv}".`,
        ],
        principles: [
          ...
          "A transition goes with the clause whose relation to what came before it names.",
        ],
        trap: "Treating the transition like and or but and joining the clauses with a comma, which creates a comma splice.",
        hint: `Could the words after "${adv}" stand alone as a sentence? And which clause does "${adv}" connect to what?`,
        estimatedSeconds: 65,
      });
      instance.verify = () => {
        if (!parts) return false;
        const lead = sentenceLead(parts.before);
        // The first clause opens the passage, so nothing precedes it for a
        // clause-final transition to link to.
        const opensPassage = lead === parts.before;
        const clauses = lead.includes(topic.c1[0]) && lead.includes(topic.c1[1]) &&
          ...
          !SUBORDINATORS.includes(firstWord(lead));
        return CONJ_ADVERBS.includes(adv) && opensPassage && clauses &&
          instance.correct === `${w1}; ${adv},` && isSquare(instance.features);
      };
      return instance;
    },
  };
  ...
  const GOVERNED_LIST_TOPICS = [
    ...
    },
    {
      ...
      text: "Anthropologist Dawa Lhamo interviewed forty mountain guides who lead climbers on Himalayan peaks. The guides ______ sickness, and inexperienced clients as the greatest dangers they face.",
    ...
    },
    {
      ...
      text: "Marine biologist Hana Kekumu counted animals in tide pools along the Oregon coast every summer for a decade. Her study focused on three ______ black oystercatcher, and the giant green anemone.",
  ...
  const CORE_PAIR_TOPICS = [
    {
      ...
      head: "bundle", w1: "Amazon", verbs: { singular: "is", plural: "are" },
    },
    {
    ...
    },
    {
      ...
      text: "Seismologist Rina Kato compared thousands of small earthquakes recorded beneath a volcano in central Japan. Her analysis of the quakes that shook the mountain in the months before its ______ that they had been creeping steadily upward.",
    ...
    },
    {
      ...
      text: "Historian Kwame Asante followed the residents of a single Philadelphia street through census records. The records that Asante traced for that street over the course of a ______ that nearly every household there took in at least one boarder in 1880.",
      head: "records", w1: "century", verbs: { singular: "shows", plural: "show" },
    },
    {
      scene: "sec-ruiz-night-photos",
      text: "Photographer Alma Ruiz spent two winters photographing people who work through the night in Chicago. Her pictures of night-shift workers taken between midnight and ______ lit only by streetlamps and the glow of vending machines.",
  ...
  // Each topic: a series whose items contain commas. The blank covers the
  // end of one item (w1), the mark between items, and the next item up to and
  // including its own internal comma: `a` + "," + `b`.
  const SERIES_TOPICS = [
    {
      scene: "sec-harmattan-film-festival",
      text: "Founded by filmmaker Chidi Okeke in 2015, the Harmattan Documentary Festival screens short films made by West African students. This fall the festival will travel to three cities: Lagos, ______; and Dakar, Senegal.",
      w1: "Nigeria", a: "Accra", b: "Ghana",
    },
    {
    ...
    },
    {
      ...
      text: "An exhibit at the maritime museum tells the stories of three nineteenth-century lighthouse keepers who were women: Ida Lewis, of Newport, Rhode Island; Abbie Burgess, of Matinicus Rock, ______ Michigan City, Indiana.",
      w1: "Maine", a: "and Harriet Colfax", b: "of",
    },
    {
      scene: "sec-mural-neighborhoods",
      text: "The murals painted for the city's 2022 arts festival each celebrate a single neighborhood. The festival paired each of three muralists with a neighborhood: Rosa Delgado, East ______; and Marcus Bell, Bedford-Stuyvesant.",
      w1: "Harlem", a: "Jin Kim", b: "Flushing",
    },
    {
      scene: "sec-meteor-showers",
      text: "Astronomer Leah Grant's guide for beginning stargazers lists three reliable meteor showers: the Quadrantids, which peak in early January; the Perseids, which peak in ______ peak in December.",
      w1: "August", a: "and the Geminids", b: "which",
    },
    {
      scene: "sec-wild-rice-sites",
      text: "Wild rice, a grass that grows in shallow lakes and slow rivers, holds deep cultural importance for the Ojibwe people. Botanist Ravi Menon collected its seeds from a lake near Duluth, ______; and a river delta near Marquette, Michigan.",
      w1: "Minnesota", a: "a marsh near Ashland", b: "Wisconsin",
    },
    {
      scene: "sec-working-lives-podcast",
      text: "The first season of the oral history podcast Working Lives featured interviews with Dolores Vega, a retired firefighter; Amos Clarke, a beekeeper in ______ pilot for a cargo airline.",
      w1: "Vermont", a: "and Fatima Zahra", b: "a",
    },
  ];
  ...
  const seriesSemicolons = {
    ...
    recognize:
      "Each item in the series contains a comma of its own, so semicolons separate the items and each item keeps its internal comma.",
    rubric: { steps: 1, concept: 1, interpretation: 1, distractors: 1, abstraction: 0, synthesis: 0, trap: 1 },
    tricks: ["neighbouring-rule", "equivalent-form"],
    build(t) {
      const topic = t.pick(SERIES_TOPICS);
      const { w1, a, b } = topic;
      const render = (separator, internal) => `${w1}${separator === "semicolon" ? ";" : ","} ${a}${internal === "comma" ? "," : ";"} ${b}`;
      const choices = squareOf(["separator", "internal"], [["semicolon", "comma"], ["comma", "semicolon"]], render, (separator, internal) => {
        const problems = [];
        if (separator === "comma") problems.push(`the items already contain commas, so a comma after "${w1}" would blur where one item ends and the next begins; the other items are separated by semicolons`);
        if (internal === "semicolon") problems.push(`a semicolon before "${b}" would split the item beginning "${a.replace(/^and /, "")}" in two; inside an item, a comma is used, as in the other items`);
        return problems.length ? cap(`${problems.join("; also, ")}.`) : null;
      });
      const instance = multipleChoice(topic, choices, {
        explanation: `Every item in the series contains a comma of its own, as the rest of the list shows, so semicolons separate the items, and the new item keeps its internal comma: "${choices.correct}".`,
        steps: [
        ...
        trap: "Choosing a comma between items because series items are usually separated by commas, without noticing that each item contains one already.",
        hint: "Look at the punctuation the passage already uses between the other items, and inside them.",
        estimatedSeconds: 60,
      });
      instance.verify = () => {
        ...
        return around(topic.text) !== null && semicolons >= 2 && /;/.test(topic.text) &&
          instance.correct === `${w1}; ${a}, ${b}` && isSquare(instance.features);
      };
      return instance;
    },
  };
  ...
  // continues with a quantifier `q` and a relative pronoun: "which" for
  // things, "whom" for people (`kind`). The choices cross the mark before
  // the relative clause (comma or semicolon) with the pronoun.
  const RELATIVE_QUANTIFIER_TOPICS = [
    ...
    },
    {
      ...
      w1: "lakes", quant: "one", kind: "thing", clause: ["Several", "hold"],
    },
    {
      ...
      w1: "students", quant: "several", kind: "person", clause: ["competition", "brought"],
    },
    {
      ...
      w1: "masks", quant: "each", kind: "thing", clause: ["Visitors", "can see"],
    },
    {
    ...
    },
    {
      ...
      w1: "crew members", quant: "many", kind: "person", clause: ["Rosa Mendes", "interviewed"],
    },
    {
      ...
      w1: "fishermen", quant: "several", kind: "person", clause: ["founders", "were"],
    },
    {
      ...
      w1: "coins", quant: "many", kind: "thing", clause: ["Divers", "have recovered"],
    },
  ];
  ...
  const relativeVsPronounClause = {
    ...
    subskill: "sentence boundaries",
    difficulty: "Medium",
    title: "Attaching a relative clause after a quantifier",
    recognize:
      "“Most of which” (things) and “most of whom” (people) begin dependent clauses, which attach to the main clause with a comma, never a semicolon.",
    ...
    build(t) {
      ...
      const needed = topic.kind === "person" ? "whom" : "which";
      const render = (mark, pronoun) => `${w1}${mark === "comma" ? "," : ";"} ${quant} of ${pronoun}`;
      const choices = squareOf(["mark", "pronoun"], [["comma", "semicolon"], ["which", "whom"]], render, (mark, pronoun) => {
        const problems = [];
        if (mark === "semicolon") problems.push(`a semicolon must join two independent clauses, but "${quant} of ${pronoun}" begins a dependent clause, which would be left as a fragment`);
        if (pronoun !== needed) {
          problems.push(pronoun === "whom"
            ? `"whom" refers to people, but the ${w1} are things`
            : `"which" refers to things, but the ${w1} are people, so the pronoun is "whom"`);
        }
        return problems.length ? cap(`${problems.join("; also, ")}.`) : null;
      });
      const instance = multipleChoice(topic, choices, {
        explanation: `"${cap(quant)} of ${needed}" begins a dependent clause describing the ${w1}, so it attaches to the main clause with a comma. Because the ${w1} are ${topic.kind === "person" ? "people" : "things"}, the pronoun is "${needed}": "${choices.correct}".`,
        steps: [
          ...
          `Use "whom" for people and "which" for things: the ${w1} are ${topic.kind === "person" ? "people" : "things"}.`,
        ],
        principles: [
          "Which, whom, and whose begin dependent clauses, even inside phrases such as most of which.",
          "A dependent clause attaches with a comma; a semicolon would leave it a fragment.",
          "Whom refers to people; which refers to things.",
        ],
        trap: "Choosing a semicolon because the words after the blank have their own verb, although “of which” or “of whom” makes them dependent.",
        hint: "Could the words after the blank stand alone as a sentence? Are the nouns before it people or things?",
        estimatedSeconds: 65,
      });
      instance.verify = () => {
        const parts = around(topic.text);
        ...
        // The clause after the blank has its own verb but no subject of its
        // own: the relative pronoun is its subject.
        const verbFirst = /^\s[a-z]+/.test(parts.after);
        return clause && verbFirst && instance.correct === render("comma", needed) && isSquare(instance.features);
      };
      return instance;
    },
  };
  ...
  // Hard. Each topic: right after `head`, a dash or an opening parenthesis
  // starts a long nonessential element that contains commas of its own and
  // nouns of the other number; it ends with w1, and the sentence's main verb
  // follows the blank. The choices cross the closing mark (the matching one
  // or the tempting comma) with the verb's number.
  const CLOSING_MARK_TOPICS = [
    ...
    },
    {
      ...
      head: "Ines Varga", subject: "composer", number: "singular", opener: "—", w1: "music", verbs: { singular: "asks", plural: "ask" },
    },
    {
    ...
    },
    {
      ...
      text: "Nineteenth-century ice houses—buildings in which, long before electric refrigerators existed, blocks of lake ice were packed in sawdust to last through the ______ still standing on old farms across New England.",
      head: "ice houses", subject: "houses", number: "plural", opener: "—", w1: "summer", verbs: { singular: "is", plural: "are" },
    },
    {
      scene: "sec-corner-store-survey",
      text: "Sociologist Denise Albright spent a year visiting corner stores in Baltimore. Her survey of these stores—small shops that, in many city neighborhoods, serve as the closest source of ______ that most of them stock fresh fruit only during the summer months.",
      head: "these stores", subject: "survey", number: "singular", opener: "—", w1: "groceries", verbs: { singular: "shows", plural: "show" },
    },
    {
      scene: "sec-axolotl",
    ...
    },
    {
      ...
      text: "The cliff dwellings of Tolan Canyon—rooms of stone and mud that, according to tree-ring dates from their roof beams, were built in the ______ visible from a trail that follows the canyon floor.",
      head: "Tolan Canyon", subject: "dwellings", number: "plural", opener: "—", w1: "1200s", verbs: { singular: "is", plural: "are" },
    },
    {
  ...
  const CLOSERS = { "—": "—", "(": ")" };
  const OPENER_NAMES = { "—": "dash", "(": "opening parenthesis" };
  const CLOSER_NAMES = { "—": "dash", "(": "parenthesis" };
  ...
  const closingMarkMatch = {
    ...
    recognize:
      "A dash or parenthesis far back opened an interruption full of commas and nouns; it must close with the same kind of mark, and the verb after it agrees with the subject before the interruption.",
    ...
    build(t) {
      ...
      const { number } = topic;
      // A dash closes with no space before the verb; a parenthesis or comma
      // is followed by one.
      const render = (mark, n) => `${w1}${mark === "match" ? closer : ","}${mark === "match" && closer === "—" ? "" : " "}${topic.verbs[n]}`;
      const choices = squareOf(["closer", "number"], [["match", "comma"], ["singular", "plural"]], render, (mark, n) => {
        const problems = [];
        if (mark === "comma") problems.push(`the interruption opened with ${opener === "(" ? "an" : "a"} ${OPENER_NAMES[opener]} after "${topic.head}", so it must close with a ${CLOSER_NAMES[opener]}; the commas inside it do not change which mark closes it`);
      ...
      const instance = multipleChoice(topic, choices, {
        explanation: `The ${OPENER_NAMES[opener]} after "${topic.head}" opens an interruption that ends with "${w1}"; it closes with a matching ${CLOSER_NAMES[opener]}, however many commas it contains. The main verb then agrees with the subject before the interruption, "${topic.subject}", which is ${number}: "${choices.correct}".`,
        steps: [
        ...
        trap: "Closing with a comma because the interruption already contains commas, or matching the verb to the plural or singular noun right before the blank.",
      ...
      instance.verify = () => {
        ...
        // The element contains commas (the lure) but no mark of its own kind.
        const lured = /,/.test(inside) && !(opener === "(" ? /[()]/ : /—/).test(inside);
  ...
  const ESSENTIAL_CLAUSE_TOPICS = [
    ...
    },
    {
      ...
      text: "A hospital in Leeds tested a new scheduling system on two of its wards while its other wards kept the old one. Three months into the trial, the ______ reporting fewer missed breaks and less unpaid overtime.",
    ...
    },
    {
      ...
      text: "Ornithologist Kofi Mensah counted barn swallows passing a headland on the coast of Ghana every day during the spring migration. The ______ by far the busiest, with flocks of more than a thousand birds passing each hour.",
    ...
    },
    {
      ...
      text: "Archaeologist Carla Benedetti sorted 2,000 glass fragments from a Roman villa by color and thickness. The ______ pieces of windows, not of drinking cups, she concluded.",
  ...
  const essentialClauseContext = {
    ...
    build(t) {
      ...
      const choices = squareOf(["commas", "number"], [["pair", "none"], ["singular", "plural"]], render, (commas, n) => {
        ...
        if (n !== number) problems.push(`"${topic.verbs[n]}" is ${n}, but it agrees with "${head}", which is ${number}`);
        return problems.length ? cap(`${problems.join("; also, ")}.`) : null;
      });
      const instance = multipleChoice(topic, choices, {
  ...
  // `head`, often holding a relative clause with its own verb) that ends
  // with w1; the transition `adv` then either interrupts that single clause
  // before its main verb w2 ("interrupt"), or, where the words before the
  // blank already form a complete clause (`c1`), begins a second clause
  // ("clause"; only "however", whose meaning changes without its comma).
  // The choices cross the mark before `adv` with the mark after it.
  const INTERRUPTER_TOPICS = [
    ...
    },
    {
      ...
      head: "The seagrass meadows", w1: "stretch", adv: "similarly", w2: "recovered", kind: "interrupt",
    },
    {
      ...
      head: "The visitors", w1: "year", adv: "instead", w2: "relied", kind: "interrupt",
    },
    {
      scene: "sec-kessel-footbridge-forecast",
      text: "The Kessel Gorge has been crossed by footbridges for more than two centuries. The engineers who built the first wooden bridges across it expected them to last for a century or ______ most of them had to be replaced within a few decades.",
      head: "The engineers", w1: "more", adv: "however", w2: "most", kind: "clause", c1: ["engineers", "expected"],
    },
    {
      scene: "sec-pell-curators-assumption",
      text: "Before its 2022 renovation, the Pell Museum surveyed visitors about how they explore its galleries. The curators who planned the renovation assumed that visitors would use the new touch-screen ______ most visitors relied on printed labels and on conversations with one another.",
      head: "The curators", w1: "guides", adv: "however", w2: "most", kind: "clause", c1: ["curators", "assumed"],
    },
    {
      scene: "sec-homework-expectations",
      text: "Many parents in the district worried when teachers began assigning shorter homework. The students whose teachers agreed to try the shorter assignments expected their grades to ______ they earned slightly higher grades than their peers by the end of the term.",
      head: "The students", w1: "fall", adv: "however", w2: "they", kind: "clause", c1: ["students", "expected"],
    },
    {
      scene: "sec-heat-damaged-seagrass",
      text: "Marine heat waves can kill large areas of seagrass in a single summer. The seagrass meadows that researchers monitored along one stretch of the Australian coast were badly damaged by a heat wave in ______ they recovered within five years, faster than anyone had predicted.",
      head: "The seagrass meadows", w1: "2011", adv: "however", w2: "they", kind: "clause", c1: ["meadows", "were badly damaged"],
    },
    {
      scene: "sec-aldridge-attribution",
      text: "The Aldridge Gallery's catalog of portraits was first compiled in 1911. The historian who compiled it attributed one of the portraits to a student of the Flemish painter Jan ______ x-rays later revealed an underdrawing in the hand of Verhulst himself.",
  ...
  const adverbInterruptsClause = {
    ...
    recognize:
      "Decide whether the words before the blank are only a long subject (the transition interrupts one clause and, when set off, takes a comma on each side) or a complete clause (the transition begins a new clause after a semicolon).",
    ...
    build(t) {
      ...
      const parts = around(topic.text);
      const lead = parts ? `${sentenceLead(parts.before)}${w1}`.trim() : "";
      const render = (before, after) => `${w1}${before === "comma" ? "," : ";"} ${adv}${after === "comma" ? "," : ""} ${w2}`;
      const choices = squareOf(["before", "after"], [["comma", "semicolon"], ["comma", "none"]], render, (before, after) => {
        const problems = [];
        if (interrupt) {
          ...
          if (after === "none") problems.push(`once a comma sets off the start of "${adv}", a second comma must close it; a single comma separates the subject from its verb`);
        } else {
          if (before === "comma") problems.push(`"${lead}" is already a complete clause (its verb is "${topic.c1[1]}"), and a new clause follows; a comma before "${adv}" joins them with only a comma, a comma splice`);
          if (after === "none") problems.push(`without a comma after it, "${adv}" reads as "in whatever way" rather than as a transition`);
        }
        return problems.length ? cap(`${problems.join("; also, ")}.`) : null;
      });
      const instance = multipleChoice(topic, choices, {
        explanation: interrupt
          ...
          : `"${lead}" is a complete clause: its main verb is "${topic.c1[1]}". A second independent clause follows, and "${adv}" begins it, so a semicolon separates the clauses and a comma follows "${adv}": "${choices.correct}".`,
        steps: [
        ...
        principles: [
          "A transition that begins a new independent clause takes a semicolon or period before it and, like however, a comma after it.",
          "A transition that interrupts a single clause, when it is set off at all, takes a comma on each side; a single comma would separate a subject from its verb.",
      ...
      instance.verify = () => {
        ...
        const key = kind === "interrupt" ? `${w1}, ${adv}, ${w2}` : `${w1}; ${adv}, ${w2}`;
        return CONJ_ADVERBS.includes(adv) && kind === topic.kind && (kind === "interrupt" || adv === "however") &&
          instance.correct === key && isSquare(instance.features);
      };
      return instance;
    },
  };
  ...
  // Hard. Each topic: the blank holds the noun before a supplement (in the
  // right number, `forms`) and the mark that opens the supplement. How the
  ...
  const APPOSITIVE_TOPICS = [
    ...
    {
      scene: "sec-whitcomb-pikas",
      text: "Pikas, small relatives of rabbits, overheat quickly in warm air, so many scientists expected them to vanish from low mountain slopes. Dana ______ has found thriving colonies well below the predicted limit.",
      kind: "appositive", left: "Whitcomb", middle: "an ecologist who studies small mammals in Nevada", right: "has",
    },
    {

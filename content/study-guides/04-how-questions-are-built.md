...
Writers don't invent random wrong answers. They construct them from **the errors
...
| --- | --- |
| Extra step | The obvious computation gives a listed wrong answer |
| Answering a different question | Solves for `x`, but asks for `2x + 1` |
| Closer distractors | Two choices differ by one word |
| Harder elimination | Every choice is partially supported |
...
- **Reading:** the choice is a true, well-supported statement about the passage
  — it just doesn't answer *this* question. It might answer the question two
  items down.
...
Built from a specific, common error.

- Sign error: you get `−7`, the answer is `7`, and both are choices
- Off-by-one: forgetting to include an endpoint
- Reciprocal or inverse: `3/4` when the answer is `4/3`
- Percent of vs. percent change vs. percent remaining
- Using diameter where radius belongs
- Forgetting to convert units
- Stopping one step early

**Defense:** when your answer appears in the list, that is *not* confirmation.
Do a quick independent check — estimate, plug back in, sanity-check magnitude
and sign.

### 3. The Scope Error

The choice is broadly related to the passage but too big, too small, or
sideways.

| Type | What it looks like |
| --- | --- |
| **Too broad** | Describes the whole passage when asked about one paragraph |
| **Too narrow** | Describes one detail when asked for the main idea |
| **Too strong** | Right direction, overstated: `proves` instead of `suggests` |
...
### 4. The Half-Right Choice

Two clauses; the first is perfectly correct and the second is false. Students
read the first half, feel recognition, and stop.

> *"The author cites the 1974 survey to establish the scale of the problem and
> to argue that later studies were methodologically flawed."*

The first half may be exactly right. If the passage never criticizes later
...
The stem tells you what kind of evidence counts. Learning to classify stems
instantly is a large speed gain.

### Reading-type stems

| Stem pattern | What it wants | Evidence standard |
| --- | --- | --- |
...
| "Which data from the graph most strongly supports..." | Quantitative evidence | Read the axes; the claim must match the data direction |
...
| "...best models the situation" | Check units and the meaning of each coefficient |
| "In the context of the model, what does 12 represent?" | Interpretation, not computation |
| "If the graph is shifted..." | Direction of transformation is the trap |

The **must/could** distinction decides a surprising number of hard items. They
require opposite strategies: *must be true* is attacked by hunting
counterexamples; *could be true* is attacked by finding one example that works.

### Writing/grammar stems
...
| "Should the writer make this addition?" | Relevance — usually no, if it's off-topic |

**On any stem that states a goal, the goal is the grading rubric.** A beautifully
written choice that doesn't accomplish the stated goal is wrong. Students lose
these by picking the best-sounding sentence rather than the one that does the
stated job.
...
A practical routine for review. When you miss something, run this:

1. **What was the tested concept?** Name it in five words.
2. **What distractor family was my choice?** Use the five above.
3. **What was the difficulty lever?** What made this harder than the easy
   version of the same concept?
4. **What was the earliest moment I could have caught it?** Usually earlier than
   you think — often at the stem, before any work.
5. **What one-sentence rule would have saved me?** Write it down. That sentence
   is your actual study material.

That last step is the whole point. Over eight weeks you accumulate 40-60
sentences that are specific to *your* failure modes. Reading that list before a
test is worth more than re-reading any guide, including this one.
...
**Reading**
- The credited answer paraphrases heavily, so it doesn't *look* like the text
- Two choices are both partially supported
- The passage's central claim is stated once, subtly, in the middle
- The question asks about structure or function rather than content

**Math**
- Multiple representations in one problem (equation + graph + table)
- The answer requires an interpretation step after the computation
- The setup is the hard part; the arithmetic is trivial
...
- Eliminate faster so you can spend time where it matters
- Catch your own errors before you commit
- Break a genuine 50/50
- Guess intelligently when the clock is gone

Do not use them to avoid learning the material. There is no substitute, and
anyone selling you one is lying.

---

**Next:** [Universal answer patterns](05-universal-answer-patterns.md) — the
specific tells, section by section.

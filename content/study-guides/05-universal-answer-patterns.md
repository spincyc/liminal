...
Concrete, checkable tells that apply across both tests. Where
...
## Tier 1 — Always applicable

### Elimination beats selection

You almost never need to *prove* the right answer. Prove three wrong and you're
done by default.

This matters because proving something wrong is a much lower bar: one false
word, one unsupported clause, one impossible sign. Hunting for the perfect
answer invites you to rationalize; hunting for flaws is mechanical.

**Practice this deliberately.** For a week, force yourself to write a reason
next to each eliminated choice. It's slow at first and then it isn't, and it
will change your accuracy on 50/50s more than anything else in this library.

**Fails when:** all four choices look defensible. That usually means you
misread the stem — go back to the question, not the choices.

### One false element kills the whole choice

Treat every answer choice as an **AND** statement. If a choice says *"X and Y,"*
both must be true. If it says *"X in order to Y,"* both the action and the
...
answering a slightly different question. Item writers know this and put the
intermediate value in the choices.

**Fails when:** never. This is a habit, not a heuristic.
...
If two options are functionally identical, neither can be credited — the item
would have two correct answers and would have been cut in review. Eliminate
both.

This is a genuinely powerful elimination and it's underused. It converts a
4-choice question into a 2-choice question instantly.

**Fails when:** they only *look* similar and there's a real distinction you're
glossing over. Check for a single differing word before eliminating both.
...
## Tier 3 — Math-specific

### Your answer being listed means nothing

Distractors are constructed *from* the errors students make. Finding your answer
among the choices is the expected outcome of making a standard mistake, not
confirmation.

Always do one independent check: substitute back, estimate, or check sign and
magnitude.

### Estimate before you compute

A ten-second estimate eliminates one to three choices on a large fraction of
math items and catches sign, magnitude, and unit errors that exact computation
will not.

If the choices are `4`, `12`, `48`, `144` and a rough estimate says "around
50," you're done without solving.

### Plug in numbers

Two versions, both essential:

**Plugging in for variables** — when the choices contain variables, pick a
concrete value (avoid `0` and `1`, which make too many things equal), compute
the target, then test which choice produces it.
...
- Did it ask for the value, or the value squared?
- Minutes or hours? Inches or feet?
- The increase, or the new total?
- Per item, or in total?
- The number of solutions, or the solutions?

A significant share of missed math questions are correct work plus a final-step
misread.
...
### Determine the logical relationship *before* reading transition choices

For transition questions, cover the choices. Read the sentence before and the
sentence after. Decide the relationship yourself: continuation, contrast, cause,
example, sequence, concession, conclusion. *Then* look for the choice matching
that relationship.

Reading the choices first is how students end up choosing a transition that
"sounds fine." Most wrong transition answers sound fine.

### On rhetorical-purpose questions, the stated goal is the entire rubric

When the stem says *"the writer wants to emphasize X"* — X is the grading
criterion. A more elegant sentence that doesn't emphasize X is wrong.

Reread the goal after you've narrowed to two. This resolves most of them
immediately.

### Punctuation: the four-way test

For SAT Boundaries and ACT Punctuation items, classify each side of the
punctuation:

1. Is what's before it an independent clause?
2. Is what's after it an independent clause?

| Before | After | Legal joins |
| --- | --- | --- |
| Independent | Independent | Period, semicolon, comma + FANBOYS, colon (if the second explains the first), dash |
| Independent | Dependent/phrase | Comma, colon, dash — or nothing |
...
## Tier 5 — Science and data specific
...
When asked what happens generally, the answer describes the trend, not one data
...
Every tell above is a *tiebreaker*. The order never changes:
...
**Next:** [Timing and pacing](06-timing-and-pacing.md)
